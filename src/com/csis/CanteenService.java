...
public final class CanteenService {
    ...
    // submitPayment method is called when a customer submits a payment for an order. 
    ...
    // verifyPayment method is called by the cashier when they verify or reject a payment. 
    ...
    // The markKitchenReady method is called when the kitchen staff marks an order as ready. 
    ...
    // The deductRecipeIngredients method is called to reduce the stock of ingredients used in a recipe. 
    // It calculates the required quantities, checks for sufficient stock, and updates the ingredient quantities and stock movement records.
    ...
    // The sendSystemNotification method inserts a new notification record into the notifications table for the user associated with the order. 
    // It uses a JOIN to find the correct user_id based on the order's customer_id and sends a message indicating that their order is ready or completed.
    ...
    // The markKitchenReady method is called when the kitchen staff marks an order as ready. 
    // It updates the order status to "READY" and sets the prep_ready_at timestamp. 
    ...
    // The markKitchenReady method is called when the kitchen staff marks an order as ready. It updates the order status to "READY" and sets the prep_ready_at timestamp. It also sends a system notification to the customer informing them that their order is ready for pickup.
    ...
    // The closeOrderServed method is called when a staff member confirms that the customer's order has been served. It updates the order status to "COMPLETED" and sets the served_completed_at timestamp. It also sends a final notification to the customer confirming that their order has been marked as completed.
    ...
    // The notifyStaffCustomerArrived method updates the customer_notified_at timestamp for the order when a staff member confirms that the customer has arrived to pick up their ready order. This can be used to track when customers are notified and potentially trigger additional actions or notifications if needed.
    ...
    // The sendSystemNotification method inserts a new notification record into the notifications table for the user associated with the order. It uses a JOIN to find the correct user_id based on the order's customer_id and sends a message indicating that their order is ready or completed.
