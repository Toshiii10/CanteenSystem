...
public final class CanteenService {
    ...
    public static void applyPromotion(int orderId, String promoCode) throws SQLException {
        ...
        try (Connection c = DB.getConnection()) {
            ...
            try (PreparedStatement check = c.prepareStatement(
                "SELECT discount_type, discount_value FROM promotions WHERE promo_code = ? AND status = 'ACTIVE' AND CURDATE() BETWEEN start_date AND end_date")) {
                check.setString(1, promoCode.trim());
                try (ResultSet rs = check.executeQuery()) {
                    if (!rs.next()) throw new SQLException("Promo code '" + promoCode + "' is invalid, inactive, or expired.");
                    type = rs.getString("discount_type");
                    discountVal = rs.getDouble("discount_value");
                }
            }
            
            double subtotal = 0;
            try (PreparedStatement getSub = c.prepareStatement("SELECT subtotal FROM orders WHERE order_id = ?")) {
                getSub.setInt(1, orderId);
                try (ResultSet rs = getSub.executeQuery()) {
                    if (rs.next()) subtotal = rs.getDouble("subtotal");
                }
            }
            
            double finalDiscountAmount = 0;
            if ("PERCENTAGE".equalsIgnoreCase(type)) {
                finalDiscountAmount = subtotal * (discountVal / 100.0);
            } else {
            ...
            try (PreparedStatement update = c.prepareStatement("UPDATE orders SET discount_amount = ? WHERE order_id = ?")) {
                update.setDouble(1, finalDiscountAmount);
    ...
    private static void recalc(Connection c, int orderId) throws SQLException {
        String sql = """
            ...
            SET subtotal = (SELECT COALESCE(SUM(line_total),0) FROM order_items WHERE order_id=o.order_id),
                total_amount = GREATEST(0, (SELECT COALESCE(SUM(line_total),0) FROM order_items WHERE order_id=o.order_id) - discount_amount),
                balance = GREATEST(0, (SELECT COALESCE(SUM(line_total),0) FROM order_items WHERE order_id=o.order_id) - discount_amount - amount_paid)
            WHERE order_id=?
            """;
   ...
   public static void verifyPayment(int paymentId, int userId, boolean accept) throws SQLException {
        try (Connection c = DB.getConnection()) {
            c.setAutoCommit(false);
            try {
                try (PreparedStatement p = c.prepareStatement("UPDATE payments SET status=?, verified_by=?, verified_at=NOW() WHERE payment_id=? AND status='PENDING'")) {
                ...
                if (accept) {
                    try (PreparedStatement o = c.prepareStatement("UPDATE orders SET payment_status='PAID' WHERE order_id = (SELECT order_id FROM payments WHERE payment_id = ?)")) {
                        o.setInt(1, paymentId);
                        o.executeUpdate();
                    }
                }
                c.commit();
    ...
    private static void refundToWallet(Connection c, int customerId, double amount, int orderId) throws SQLException {
        ...
        try (PreparedStatement wCheck = c.prepareStatement("SELECT wallet_id FROM wallets WHERE customer_id = ? FOR UPDATE")) {
    ...
    public static void processWalletPayment(int orderId, int customerId) throws SQLException {
        try (Connection c = DB.getConnection()) {
            ...
            try {
                ...
                // THE PADLOCK: Row locking enforced here
        ...
        try (Connection c = DB.getConnection()) {
            ...
            try (PreparedStatement cap = c.prepareStatement("SELECT max_order_capacity FROM pickup_slots WHERE slot_id = ?")) {
                ...
                try (ResultSet capRs = cap.executeQuery()) {
                    if (capRs.next()) {
                        maxCapacity = capRs.getInt("max_order_capacity");
                    } else {
                        return false; 
                    }
                }
            ...
            int booked = 0;
            try (PreparedStatement p = c.prepareStatement(
                    "SELECT COUNT(*) FROM orders WHERE slot_id = ? AND DATE(ordered_at) = ? AND order_status NOT IN ('CANCELLED', 'REJECTED')")) {
                p.setInt(1, slotId);
    ...
    public static void closeOrderServed(int orderId) throws SQLException {
        try (Connection c = DB.getConnection();
            ...
            p.setInt(1, orderId);
            if (p.executeUpdate() == 0) throw new SQLException("Order ID " + orderId + " not found.");
            sendSystemNotification(orderId, "Your Canteen Order has been marked as completed. Thank you for your order!");
        }
    }
    
    public static void notifyStaffCustomerArrived(int orderId, int customerUserId) throws SQLException {
        try (Connection c = DB.getConnection();
             PreparedStatement p = c.prepareStatement("UPDATE orders o JOIN customer_profiles cp ON cp.customer_id = o.customer_id SET o.customer_notified_at = NOW() WHERE o.order_id = ? AND cp.user_id = ? AND o.order_status = 'READY'")) {
            p.setInt(1, orderId);
            p.setInt(2, customerUserId);
            p.executeUpdate();
        }
    }
    ...
    public static void cancelAndRefundOrder(int orderId) throws SQLException {
        try (Connection c = DB.getConnection()) {
            c.setAutoCommit(false); 
            try {
                int cid = -1;
                ...
                try (PreparedStatement getO = c.prepareStatement("SELECT customer_id, payment_status, order_status FROM orders WHERE order_id = ? FOR UPDATE")) {
                    getO.setInt(1, orderId);
                    ResultSet rs = getO.executeQuery();
                    if (rs.next()) {
                        cid = rs.getInt("customer_id");
                        paymentStatus = rs.getString("payment_status");
                        orderStatus = rs.getString("order_status");
                    } else throw new SQLException("Order ID " + orderId + " not found.");
                }

                if ("CANCELLED".equals(orderStatus)) {
                    throw new SQLException("This order has already been cancelled and refunded.");
                }

                int totalQty = 0;
                double totalCost = 0.0;
                try (PreparedStatement getItems = c.prepareStatement("SELECT SUM(quantity) AS qty, SUM(line_total) AS cost FROM order_items WHERE order_id = ?")) {
                ...
                if ("PAID".equals(paymentStatus)) {
                    ...
                    int wId = -1;
                    try (PreparedStatement checkW = c.prepareStatement("SELECT wallet_id FROM wallet_transactions WHERE reference_order_id = ? AND txn_type = 'DEDUCT' LIMIT 1")) {
                        checkW.setInt(1, orderId);
                        ResultSet rs = checkW.executeQuery();
                        if (rs.next()) {
                            isWallet = true;
                    ...
                    if (isWallet) {
                        try (PreparedStatement updW = c.prepareStatement("UPDATE wallets SET balance = balance + ? WHERE wallet_id = ?")) {
                            updW.setDouble(1, totalCost);
                            updW.setInt(2, wId);
                            updW.executeUpdate();
                        }
                        try (PreparedStatement logW = c.prepareStatement("INSERT INTO wallet_transactions (wallet_id, amount, txn_type, description, reference_order_id) VALUES (?, ?, 'REFUND', ?, ?)")) {
                            logW.setInt(1, wId);
                            logW.setDouble(2, totalCost);
                            logW.setString(3, "Automated Refund for Cancelled Order #" + orderId);
                            logW.setInt(4, orderId);
                            logW.executeUpdate();
                        }
                    } else {
                        try (PreparedStatement refSub = c.prepareStatement("UPDATE customer_subscriptions SET credits_used = GREATEST(0, credits_used - ?) WHERE customer_id = ? AND status='ACTIVE' ORDER BY end_date DESC LIMIT 1")) {
                            refSub.setInt(1, totalQty);
                            refSub.setInt(2, cid);
                            refSub.executeUpdate(); 
                        }
                    }
                }

                try (PreparedStatement cancel = c.prepareStatement("UPDATE orders SET order_status = 'CANCELLED', payment_status = 'REFUNDED' WHERE order_id = ?")) {
