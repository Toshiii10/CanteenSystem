...
// This is the main panel for the user to view the menu, add items to their cart, and checkout. 
// It includes dietary and allergen checks based on the user's profile.
public class UserOrderPanel extends JPanel {
    ...
    // Constructor for the UserOrderPanel. Initializes the UI components and loads the menu data.
    ...
    // Loads the menu data from the database, active pickup slots, and 
    // generates personalized recommendations based on the user's dietary profile.
    ...
    // Loads the active pickup slots from the database and populates the slotComboBox.
    ...
    // Generates personalized menu recommendations based on the user's dietary profile and allergen restrictions.
    ...
    // Adds the selected menu item to the cart, checking for dietary and allergen conflicts based on the user's profile.
    private void addToCart() {
        ...
        try {
            ...
            // Check dietary profile and allergen restrictions
            try (Connection c = DB.getConnection()) {
    ...
    // Processes the checkout by validating the cart, selected slot, and payment method, then creates an order in the database.
    private void processCheckout() {
        ...
        //Translate string to Priority Number for Kitchen Display
        int priorityNum = 3; 
        ...
        try {
            ...
            try {
                // THE FIX: Added queue_priority to the database update
                try (Connection c = DB.getConnection(); PreparedStatement ps = c.prepareStatement("UPDATE orders SET slot_id = ?, customer_type = ?, queue_priority = ? WHERE order_id = ?")) {
                    ps.setInt(1, slotId); ps.setString(2, priorityStr); ps.setInt(3, priorityNum); ps.setInt(4, oid); ps.executeUpdate();
                }
                for (int i = 0; i < cartModel.getRowCount(); i++) {
                ...
                // THE FIX: Checked exact strings with spaces matching the UI
