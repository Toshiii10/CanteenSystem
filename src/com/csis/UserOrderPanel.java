...
public class UserOrderPanel extends JPanel {
    ...
    private final JComboBox<String> paymentGateway = new JComboBox<>(new String[]{
        "CANTEEN_PREPAID_WALLET", "MEAL_SUBSCRIPTION_CREDIT", "MANUAL_EXTERNAL_GATEWAY"
    });
    ...
    public UserOrderPanel(UserSession session) {
        ...
        topRow.add(UIUtils.title("Kiosk Order Menu"));
    ...
    private void loadData() {
        try {
            menuModel.load("SELECT product_id, product_name, unit_price, calories, sugar, sodium, status FROM products WHERE status='AVAILABLE' ORDER BY product_name");
            loadActiveSlots();
    ...
    private void processCheckout() {
        ...
        if (slotComboBox.getSelectedItem() == null) {
            UIUtils.info(this, "Please select a pickup time slot from the 'Slot' dropdown.");
            return;
        }
