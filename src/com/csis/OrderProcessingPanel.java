...
public class OrderProcessingPanel extends JPanel {
   ...
   private void loadKitchenQueue() {
        try {
            // FIX: Filters out 'PENDING_PAYMENT' so unpaid GCash orders stay hidden from the cooks!
            kitchenModel.load("""
                SELECT order_id, order_no, scheduled_pickup, customer_type, order_status, total_estimated_prep_minutes AS 'Est Duration' 
                FROM v_kitchen_display_queue 
                WHERE order_status != 'PENDING_PAYMENT' 
                ORDER BY queue_priority ASC, scheduled_pickup ASC
                """);
