...
public class OrderProcessingPanel extends JPanel {
    ...
    // Builds the upper section of the panel that displays the kitchen queue and controls for starting preparation and marking orders as ready.
   ...
   private void loadKitchenQueue() {
        try {
            // THE FIX: We JOIN the order_items table and use GROUP_CONCAT to pull all special instructions
            // into a single readable column without duplicating the order rows!
            kitchenModel.load("""
                SELECT v.order_id, v.order_no, v.scheduled_pickup, v.customer_type, v.order_status, 
                       v.total_estimated_prep_minutes AS 'Est Duration',
                       COALESCE(GROUP_CONCAT(NULLIF(TRIM(oi.special_instruction), '') SEPARATOR ' | '), 'None') AS 'Special Instructions'
                FROM v_kitchen_display_queue v
                LEFT JOIN order_items oi ON v.order_id = oi.order_id
                WHERE v.order_status != 'PENDING_PAYMENT' 
                GROUP BY v.order_id, v.order_no, v.scheduled_pickup, v.customer_type, v.order_status, v.total_estimated_prep_minutes, v.queue_priority
                ORDER BY v.queue_priority ASC, v.scheduled_pickup ASC
                """);
