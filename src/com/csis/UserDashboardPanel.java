...
public class UserDashboardPanel extends JPanel {
    ...
    private final JLabel lblStats = new JLabel("My Total Orders: 0 | Outstanding Balance: PHP 0.0");
    ...
    public UserDashboardPanel(UserSession session) {
        ...
        JLabel lblNutriHeader = new JLabel("Daily Nutritional Intake Summary (Today's Orders)");
    ...
    private void addNutriRow(JPanel container, String title, JLabel valueLabel) {
        ...
        titleLabel.setFont(new Font("Tahoma", Font.BOLD, 15)); // Boosted row headers to 15pt Bold
        titleLabel.setForeground(new Color(33, 37, 41));
        
        valueLabel.setFont(new Font("Tahoma", Font.PLAIN, 15)); // Boosted text numbers to 15pt Plain
    ...
    private void loadData() {
        try (Connection c = DB.getConnection()) {
            // Fetch total client purchase values
            try (PreparedStatement ps = c.prepareStatement(
                "SELECT COUNT(*), COALESCE(SUM(total_amount), 0) FROM orders WHERE customer_id = (SELECT customer_id FROM customer_profiles WHERE user_id = ?)")) {
                ps.setInt(1, s.userId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        lblStats.setText("My Total Orders: " + rs.getInt(1) + " | Outstanding Balance: PHP " + String.format("%.1f", rs.getDouble(2)));
                    }
                }
            }

            // Fetch nutrition totals tracking metrics for orders placed today
            try (PreparedStatement ps = c.prepareStatement(
                ...
                "WHERE o.customer_id = (SELECT customer_id FROM customer_profiles WHERE user_id = ?) AND DATE(o.ordered_at) = CURDATE()")) {
                ps.setInt(1, s.userId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
            ...
            lblStats.setText("Metrics tracking temporary offline.");
        }
    }
}