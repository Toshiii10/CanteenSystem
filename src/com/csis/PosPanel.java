...
public class PosPanel extends JPanel {
    ...
    private void loadMenu() {
        try { m.load("SELECT product_id, product_name, unit_price, status FROM products ORDER BY product_id ASC"); } catch (Exception e) { UIUtils.error(this, e); }
    }
    
    private void doSearch() {
        String term = "%" + searchField.getText().trim() + "%";
        try { m.load("SELECT product_id, product_name, unit_price, status FROM products WHERE product_name LIKE ? ORDER BY product_id ASC", term); } catch (Exception e) { UIUtils.error(this, e); }
    }
