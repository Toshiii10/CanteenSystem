...
public class UserProfilePanel extends JPanel {
    ...
    // TextField with Dropdown for Allergens
    ...
    public UserProfilePanel(UserSession s) {
        ...
        g.gridx = 0; g.gridy = labels.length; add(save, g);
    ...
    // Loads the user's profile information from the database and populates the form fields, 
    // including handling of dietary preferences and allergen restrictions.
    private void load() {
        try (Connection c = DB.getConnection(); PreparedStatement p = c.prepareStatement(
                "SELECT u.full_name,u.email,u.phone,u.address,c.student_employee_no,c.course_department,c.dietary_notes,c.dietary_profile,c.allergen_restrictions " +
            ...
            if (r.next()) {
                ...
                // Format DB allergy to Dropdown match
    ...
    // Saves the updated profile information to the database, including handling of dietary preferences and allergen restrictions.
    private void save() {
        Connection c = null;
        try {
            ...
            PreparedStatement p = c.prepareStatement(
                "UPDATE customer_profiles SET student_employee_no=?,course_department=?,dietary_notes=?,dietary_profile=?,allergen_restrictions=? WHERE user_id=?");
            ...
            // Format Dropdown selection to Database string
            ...
            p.setInt(6, s.userId); p.executeUpdate();
            
            c.commit();
            Audit.log(s, "UPDATE_PROFILE", "Updated profile nutrition preferences");
